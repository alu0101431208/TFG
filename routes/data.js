...
// Ruta para obtener el nivel de agua más reciente
router.get('/water-level/latest', (req, res) => {
    db.get("SELECT level, timestamp FROM water_levels ORDER BY timestamp DESC LIMIT 1", [], (err, row) => {
        if (err) {
            console.error('Error al obtener el nivel de agua más reciente:', err);
            res.status(500).json({ error: err.message });
            return;
        }
        console.log('Nivel de agua más reciente:', row);
        res.json(row);
    });
});
...
router.get('/download-data', (req, res) => {
    ...
    const sql = `SELECT id, level, timestamp FROM water_levels WHERE timestamp BETWEEN ? AND ? ORDER BY timestamp`;
    ...
    db.all(sql, [startDate, endDate], (err, rows) => {
        ...
        if (rows.length > 0) {
            ...
            const formattedRows = rows.map(row => ({
                id: row.id,
                level: row.level.toFixed(5).replace('.', ','), // Convertir a cadena de texto con dos decimales
